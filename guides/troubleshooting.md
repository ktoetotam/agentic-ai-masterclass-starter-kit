...
| Git is missing or too old | Ask Codex to use `$masterclass-setup`: portable Git on Windows, user-folder Git on Mac. Restart ChatGPT after updating your user PATH. If company policy blocks installation, tell the facilitator and use folder copies temporarily; the full setup remains incomplete. |
| Mac asks to install Apple developer tools when running Git | Cancel that prompt for this user-only route. Ask Codex to use `$masterclass-setup`, put the user Git folder first on PATH, and restart ChatGPT. |
